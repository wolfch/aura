...
({
	...
	testMultipleCarousels : {
		...
		test : function(cmp) {
			...
			$A.test.assertFalse($A.util.isUndefinedOrNull(carousel1),
					"Did not find carousel 1");
			var carousel2 = cmp.find("carousel2");
			$A.test.assertFalse($A.util.isUndefinedOrNull(carousel2),
					"Did not find carousel 2");
		}
	},

	/**
	 * Switching pages using the navigation page indicators correctly switchs
	 * page to the target page and navigation indicators are updated.
	 */
	testSwitchPageUsingPageIndicator : {
		...
		test : [
				...
				function(cmp) {
					...
					this.verifyMdmPage(this.getSelectedPage(carousel), 1,
							"New Post 1");
				...
				function(cmp) {
					// go to last page
					this.goToPageOnCarousel(cmp, "carousel1", 7,
							"c1-updateOutput");
				},
				function(cmp) {
					// verify
					...
					$A.test
							.assertEquals("i have something to say...",
									outputCmp[2].get("v.value"),
		...
		test : [
				...
				function(cmp) {
					...
					$A.test.addWaitForWithFailureMessage(true, function() {
						...
						$A.test.assertNotNull(page, "Page retrieved is null");
						var outputCmp = page.get("v.body");
						$A.test.assertFalse($A.util
								.isUndefinedOrNull(outputCmp),
								"Could not find output component");
	...
	testCarouselWithOneTab : {
		...
		test : function(cmp) {
			...
			var indicatorItems = cmp.find("carousel2").find("navContainer")
					.get("v.body")[0].find("indicatorItems");
			$A.test.assertNull(indicatorItems.getElement(),
					"Should be zero tab indicators displayed");
		}
	},
