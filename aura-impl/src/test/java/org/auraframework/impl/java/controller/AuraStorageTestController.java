...
public class AuraStorageTestController implements Controller {
    ...
    @AuraEnabled
    public List<Component> getBaseball(@Key("testName") String testName) throws Exception {
        ...
        if (currentCount.intValue() % 2 == 0) {
            ...
            Component cmp = instanceService.getInstance("auraStorageTest:playerFacet", ComponentDef.class,
                    attr);
            ret.add(cmp);
            attr.put("name", "PSandavol");
            attr.put("nickName", "Panda");
            cmp = instanceService.getInstance("auraStorageTest:playerFacet", ComponentDef.class, attr);
            ret.add(cmp);
        }
