...
public class JavascriptComponentClass extends BaseJavascriptClass {
    ...
    @Override
    protected boolean hasJsCode() {
    	return !isJsCodeEmpty(controllerDef) || !isJsCodeEmpty(helperDef) || !isJsCodeEmpty(rendererDef) || !isJsCodeEmpty(providerDef);
    }
    ...
    private void writeObjectLiteral(StringBuilder out) throws IOException {
        ...
        if (controllerDef != null) {
            ...
            json.writeLiteral(controllerDef.getJsCode());
        }

        if (helperDef != null) {
        	json.writeMapKey("helper");
            json.writeLiteral(helperDef.getJsCode());
        }

        if (rendererDef != null) {
        	json.writeMapKey("renderer");
            json.writeLiteral(rendererDef.getJsCode());
        }

        if (providerDef != null) {
        	json.writeMapKey("provider");
            json.writeLiteral(providerDef.getJsCode());
        }
