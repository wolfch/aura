...
public class JavascriptTokenizer {
    ...
    public JavascriptTokenizer(DefDescriptor<? extends RootDefinition> parentDescriptor, String jsCode, Location location) {
    ...
    public void process(JavascriptCodeBuilder builder) {
        if (!AuraTextUtil.isNullEmptyOrWhitespace(jsCode)) {
	    	parseLabels(builder);
    ...
    private void parseLabels(JavascriptCodeBuilder builder)  {
        Matcher matcher = LABEL_GVP_PATTERN.matcher(jsCode);
        while (matcher.find()) {
    ...
    private void addDependencies(JavascriptCodeBuilder builder)  {
        Matcher matcher = QUALIFIED_NAME_PATTERN.matcher(jsCode);
        while (matcher.find()) {
