...
public abstract class BaseJavascriptClass {
	...
	private String minifiedJsCode;
	private List<JavascriptProcessingError> jsCodeErrors;
     ...
     */
    protected abstract boolean hasJsCode();
     ...
     */
    protected boolean isJsCodeEmpty(CodeDefinition descriptor) {
    	return descriptor == null || AuraTextUtil.isNullEmptyOrWhitespace(descriptor.getJsCode());
    }

    private void minimize(String filename) {
	    try {
	    	StringWriter sw = new StringWriter();
	    	jsCodeErrors = JavascriptWriter.CLOSURE_SIMPLE.compress(jsCode, sw, filename);
		    minifiedJsCode = sw.toString();
	    } catch (IOException e) {
    ...
    private void exportAttibutes(JavascriptCodeBuilder codeBuilder) {
    	codeBuilder.setJsCode(jsCode);
    	codeBuilder.setMinifiedJsCode(minifiedJsCode);
    	codeBuilder.setJsCodeErrors(jsCodeErrors);
    }
 }
